...
public interface TagAssignmentRepository extends JpaRepository<TagAssignmentEntity, Long> {
	...
	@Query("SELECT ta.entityId AS entityId, ta.tag AS tag FROM TagAssignmentEntity ta WHERE ta.entityType = :entityType AND ta.createdBy = :createdBy AND ta.tag.id = :tagId")
	List<Object[]> getTagsByTagIdAndByCreatedBy(Long tagId, Long createdBy, String entityType);
	...
	@Query("SELECT ta.tag.id, COUNT(ta.entityId) as count FROM TagAssignmentEntity ta GROUP BY ta.tag ORDER BY ta.tag.id")
	List<Object[]> getTotalEntityCountByTag();
	
	@Query("SELECT ta.tag.id, COUNT(ta.entityId) as count, ta.entityType AS entityType FROM TagAssignmentEntity ta GROUP BY ta.tag, ta.entityType ORDER BY ta.tag.id")
